...
import type { LibraryComponent } from './types.js';
import { createId } from './dom.js';

function normalizedPrefix(component: ComponentNode): string {
  const candidate = component.designator.replace(/[^A-Za-z]/gu, '').toUpperCase();
  if (candidate) return candidate.slice(0, 2);
...
function nextDesignator(component: ComponentNode, document: EditorDocument): string {
...
export function instantiateLibraryComponent(definition: LibraryComponent, point: Point, document: EditorDocument): ComponentNode {
  ...
  for (const port of component.ports) {
    if (port.bankId) bankLookup.get(port.bankId)?.portIds.push(port.id);
  }
  return component;
}

export function duplicateComponent(source: ComponentNode, document: EditorDocument, offset: Point = { x: 40, y: 40 }): ComponentNode {
  ...
  const bankLookup = new Map(component.pinBanks.map((bank) => [bank.id, bank]));
  for (const port of component.ports) if (port.bankId) bankLookup.get(port.bankId)?.portIds.push(port.id);
...
export interface CustomComponentInput {
  ...
  ports: Array<{
    ...
    detail?: string;
    side: Side;
    electricalClass: PortSpec['electricalClass'];
    maximumConnections: number;
  }>;
}

export function buildCustomLibraryComponent(input: CustomComponentInput): LibraryComponent {
...
export function createFreeLabel(point: Point, text = 'Annotation'): LabelNode {
  return {
    id: createId('label'),
    text,
