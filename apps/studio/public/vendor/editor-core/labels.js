import { add, clamp, inflateRect, intersectionArea, pointAtPolylineFraction, polylineIntersectsRect, rect, rectBottom, rectCenter, rectRight, rectsIntersect, unionRects, } from './geometry.js';
import { DEFAULT_TEXT_STYLE } from './component.js';
function readableAngle(tangent) {
    let angle = (Math.atan2(tangent.y, tangent.x) * 180) / Math.PI;
    if (angle > 90 || angle < -90)
        angle += 180;
    return angle;
}
function resolveTextStyle(label) {
    return { ...DEFAULT_TEXT_STYLE, ...label.style };
}
function labelSize(label, measurer) {
...
function boundsAtCenter(center, size) {
    return rect(center.x - size.width / 2, center.y - size.height / 2, size.width, size.height);
}
function componentCandidateCenter(bounds, size, candidate, gap) {
...
function componentCandidates(label, geometry, size, gap) {
    const order = label.anchor.preferredCandidates?.length
...
function wireCandidates(label, route, size, gap) {
...
function pointCandidates(label, point, size, gap) {
    const centers = [
...
function manualCandidate(label, anchorPoint, size) {
...
function resolveAnchor(label, context) {
...
function candidateScore(candidate, label, context, placed) {
    ...
    if (previous) {
        ...
        if (previous.candidate === candidate.componentCandidate)
            score -= 18;
    }
    return { score, collisions: [...new Set(collisions)] };
}
export function placeLabel(label, measurer, context, alreadyPlaced = {}) {
...
export function placeLabels(labels, measurer, context) {
    const placed = {};
    const ordered = labels
        .filter((label) => label.visible)
        .sort((a, b) => b.priority - a.priority || a.id.localeCompare(b.id));
    for (const label of ordered)
        placed[label.id] = placeLabel(label, measurer, context, placed);
    return placed;
}
export function labelPlacementBounds(placements) {
 ...
 */
export function pinLabelToWorld(label, worldPosition) {
    return {
        ...label,
        mode: 'world-pinned',
...
export function resetLabelToAutomatic(label) {
    const reset = {
        ...label,
        mode: 'auto',
        offset: { x: 0, y: 0 },
