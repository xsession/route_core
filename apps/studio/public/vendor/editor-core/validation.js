...
function endpointEntityIds(endpoint) {
    if (endpoint.kind === 'port')
        return [endpoint.componentId, endpoint.portId];
...
export function validateDocument(document, componentGeometries = {}, labelPlacements = {}) {
    const issues = [];
    ...
    for (const label of Object.values(document.labels)) {
        ...
        if (label.anchor.ownerId) {
            const ownerExists = label.anchor.ownerKind === 'component' || label.anchor.ownerKind === 'group' || label.anchor.ownerKind === 'port'
