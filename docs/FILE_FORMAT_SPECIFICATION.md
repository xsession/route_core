...
| Table | Purpose |
|---|---|
| `layout_document` / `layout_sheet` / `layout_element` | The on-canvas manufacturing drawing. `layout_element.element_kind` is one of `dimension`, `note`, `title_block`, `bom_table`, `wire_schedule`, `cut_list`, `connection_table`, `continuity_table`, `revision_table`, `custom`. Table elements are **query-backed**: they store a small `query_json` descriptor and are re-populated from live project data on read. A `revision_table` element may carry `content.excludedRevisionIds` (an array of `revision.id` values): listed revisions are omitted from the table rows and from any derived table export, giving per-row include/exclude control. |
...
### 6.1b Formboard PDF — `formboard-pdf` export

The same data is rendered to a PDF with **one page per panel** (a `rows × columns` grid produces `rows * columns` pages). Each page contains:

- a header (`FORMBOARD PANEL <n> / <total>`, project/model name, export timestamp);
- the panel drawn at 1:1 physical size with the grid, and each wire whose geometric midpoint falls in that panel drawn to scale (`to-scale` wires solid, `not-to-scale` wires dashed) with an end label;
- a per-panel wire schedule table (`WIRE, SIGNAL, FROM, TO, ROUTED MM, SET MM, BENDS, STATE`);
- a totals footer (wire count, routed/set totals, bend count, to-scale count, panel and bend/step/tolerance parameters).

The PDF is generated by a dependency-free writer (base-14 Helvetica, PDF 1.4). Wires with no routed geometry are distributed across panels in order.
...
- **Cut list** (`cut-list-csv`): `Item, Wire ID, Label, Signal, Kind, From, To, Routed length, Cut/set length, Bends, Formboard state, Unit, Color pattern, Routing pattern, Route status` — per conductor, with the set length from §6.1.
