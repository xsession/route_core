import { EPSILON, add, clamp, countPolylineCrossings, distance, inflateRect, lineSegmentsCross, manhattanDistance, normalize, orthogonalizePolyline, pointsEqual, polylineIntersectsRect, polylineLength, rectBottom, rectContainsPoint, rectRight, routeSegments, roundedOrthogonalPath, segmentAxis, segmentIntersectsRect, sideNormal, simplifyPolyline, snap, subtract, } from './geometry.js';
...
export function resolveEndpoint(endpoint, componentGeometries) {
...
function endpointLead(endpoint, options) {
...
function routeDogleg(start, end, axis, coordinate) {
    if (axis === 'horizontal') {
...
function routeTrunk(start, end, axis, coordinate) {
    return routeDogleg(start, end, axis, coordinate);
}
function uniqueSorted(values, grid) {
...
function routeSegmentBlocked(start, end, obstacles) {
...
function proximityCost(start, end, obstacles, weight) {
    if (weight <= 0)
        return 0;
...
function crossingCost(start, end, existingRoutes, crossingPenalty) {
    if (!existingRoutes || crossingPenalty <= 0)
        return 0;
    let count = 0;
    for (const route of existingRoutes) {
        for (let index = 0; index < route.points.length - 1; index += 1) {
            if (lineSegmentsCross(start, end, route.points[index], route.points[index + 1]))
                count += 1;
        }
    }
...
class MinQueue {
    ...
    push(value) {
        ...
        while (index > 0) {
            ...
            if (this.heap[parent].priority <= value.priority)
                break;
    ...
    pop() {
        ...
        while (true) {
            ...
            if (right < this.heap.length && this.heap[right].priority < this.heap[left].priority)
                child = right;
            if (this.heap[child].priority >= last.priority)
                break;
...
function createVisibilityGrid(start, end, obstacles, options, forcedCoordinates = []) {
    ...
    for (let row = 0; row < yValues.length; row += 1) {
        ...
        for (let column = 0; column < xValues.length; column += 1) {
            ...
            const candidate = { x, y };
            if (pointBlocked(candidate, hardObstacles) && !pointsEqual(candidate, start) && !pointsEqual(candidate, end))
                continue;
    ...
    for (const indices of rowMap.values())
        indices.sort((a, b) => nodes[a].point.x - nodes[b].point.x);
    for (const indices of columnMap.values())
        indices.sort((a, b) => nodes[a].point.y - nodes[b].point.y);
...
function neighboringNodes(nodeIndex, grid, obstacles) {
    ...
    for (const adjacentPosition of [rowPosition - 1, rowPosition + 1]) {
        ...
        if (adjacent === undefined)
            continue;
        if (!routeSegmentBlocked(node.point, grid.nodes[adjacent].point, obstacles))
            neighbors.push({ nodeIndex: adjacent, direction: 'horizontal' });
    }
    ...
    for (const adjacentPosition of [columnPosition - 1, columnPosition + 1]) {
        ...
        if (adjacent === undefined)
            continue;
        if (!routeSegmentBlocked(node.point, grid.nodes[adjacent].point, obstacles))
            neighbors.push({ nodeIndex: adjacent, direction: 'vertical' });
    }
    return neighbors;
}
function reconstructPath(stateKey, parent, stateNode, nodes) {
    const reversed = [];
    let current = stateKey;
    while (current) {
        const nodeIndex = stateNode.get(current);
        if (nodeIndex !== undefined)
            reversed.push({ ...nodes[nodeIndex].point });
        current = parent.get(current);
    }
    return simplifyPolyline(reversed.reverse());
}
function stateKey(nodeIndex, direction) {
    return `${nodeIndex}|${direction}`;
}
function routeOrthogonalVisibility(start, end, obstacles, options, context) {
    if (pointsEqual(start, end))
        return [{ ...start }];
    if (segmentAxis(start, end) !== 'diagonal' && !routeSegmentBlocked(start, end, obstacles))
        return [{ ...start }, { ...end }];
    const grid = createVisibilityGrid(start, end, obstacles, options);
    const queue = new MinQueue();
    const best = new Map();
    const parent = new Map();
    const stateNode = new Map();
    ...
    while (queue.size > 0 && expanded < options.maxSearchNodes) {
        ...
        if (currentCost === undefined)
            continue;
        if (current.nodeIndex === grid.endIndex)
            return reconstructPath(current.stateKey, parent, stateNode, grid.nodes);
        expanded += 1;
        for (const neighbor of neighboringNodes(current.nodeIndex, grid, obstacles)) {
            ...
            if (!options.allowCrossings && crossCost > 0)
                continue;
            ...
            if (nextCost + EPSILON >= (best.get(nextKey) ?? Number.POSITIVE_INFINITY))
                continue;
...
function hardWaypointConstraints(constraints) {
    return constraints
...
function softAvoidObstacles(constraints) {
    return constraints
...
function routePatternSegment(pattern, start, end, obstacles, options, context) {
    if (pattern === 'direct')
        return { points: routeDirect(start, end), fallback: false };
    if (pattern === 'horizontal-first')
        return { points: routeHorizontalFirst(start, end), fallback: false };
    if (pattern === 'vertical-first')
        return { points: routeVerticalFirst(start, end), fallback: false };
    if (pattern === 'dogleg-horizontal')
        return { points: routeDogleg(start, end, 'horizontal', options.doglegCoordinate), fallback: false };
    if (pattern === 'dogleg-vertical')
        return { points: routeDogleg(start, end, 'vertical', options.doglegCoordinate), fallback: false };
    if (pattern === 'trunk-horizontal')
        return { points: routeTrunk(start, end, 'horizontal', options.trunkCoordinate), fallback: false };
    if (pattern === 'trunk-vertical')
        return { points: routeTrunk(start, end, 'vertical', options.trunkCoordinate), fallback: false };
    if (pattern === 'manual')
        return { points: routeHorizontalFirst(start, end), fallback: false };
    const routed = routeOrthogonalVisibility(start, end, obstacles, options, context);
    if (routed)
        return { points: routed, fallback: false };
...
function appendPath(target, addition) {
    ...
    for (let index = startIndex; index < addition.length; index += 1)
        target.push({ ...addition[index] });
}
function routeThroughWaypoints(start, end, waypoints, pattern, obstacles, options, context) {
...
export function routeWire(wire, context) {
    const options = { ...DEFAULT_ROUTING_OPTIONS, ...wire.routing, constraints: [...(wire.routing.constraints ?? [])] };
    ...
    if (!pointsEqual(source.point, sourceLead))
        points.push(sourceLead);
    appendPath(points, core);
    if (!pointsEqual(targetLead, target.point))
    ...
    const existingCrossings = (context.existingRoutes ?? []).reduce((total, route) => total + countPolylineCrossings(simplified, route.points), 0);
    ...
    void rounded.path;
    if (obstacleViolations.length > 0)
        diagnostics.push(`Route intersects ${obstacleViolations.length} obstacle keep-out region(s).`);
    const status = obstacleViolations.length > 0 ? 'invalid' : fallback ? 'fallback' : 'valid';
    return {
        points: simplified,
...
export function findWireCrossings(wires) {
    const crossings = [];
 ...
 */
export function routePinBankFanout(pairs, options) {
    ...
    const center = options.centerCoordinate ?? (options.orientation === 'horizontal'
...
export function moveOrthogonalSegment(points, segmentIndex, delta) {
...
export function createWaypointConstraint(id, point, locked = false) {
    return {
        id,
...
export function nearestCardinalSide(vector) {
    if (Math.abs(vector.x) >= Math.abs(vector.y))
        return vector.x >= 0 ? 'east' : 'west';
    return vector.y >= 0 ? 'south' : 'north';
}
export function suggestRoutePattern(source, target) {
...
export function validateRouteClearance(points, obstacles, clearance) {
    return obstacles
