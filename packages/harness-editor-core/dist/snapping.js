...
function bestCandidate(candidates, tolerance) {
    return candidates
...
function guideFromCandidate(candidate) {
 ...
 * rounding drift.
 */
export function snapComponentDrag(geometries, movingComponentIds, proposedDelta, options) {
    ...
    return {
        ...
        correction,
        guides: [bestX, bestY].filter((candidate) => Boolean(candidate)).map(guideFromCandidate),
