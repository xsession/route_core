...
export function parseHexColor(value: string): RgbColor | undefined {
  ...
  if (!/^[0-9a-f]{3,8}$/iu.test(normalized)) return undefined;
  if (normalized.length === 3 || normalized.length === 4) {
    const digits = normalized.split('').map((digit) => digit + digit).join('');
    return parseHexColor(`#${digits}`);
  }
  if (normalized.length !== 6 && normalized.length !== 8) return undefined;
...
export function toHexColor(color: RgbColor, includeAlpha = false): string {
...
export function bestTextColor(background: string, light = '#ffffff', dark = '#111827'): string {
 ...
 */
export function resolveWirePaint(
  style: WireStyle,
  theme: EditorTheme,
  state: { selected?: boolean; hovered?: boolean; warning?: boolean; invalid?: boolean } = {},
): WirePaintResolution {
  ...
  switch (style.pattern.kind) {
    ...
    case 'shield':
      ...
      if (style.pattern.core) layers.push({ color: style.pattern.core, width: style.width, opacity, lineCap: style.lineCap });
      break;
    case 'custom':
      layers.push(...style.pattern.layers.map((layer) => ({ ...layer, opacity: (layer.opacity ?? 1) * opacity })));
      break;
  }
  const result: WirePaintResolution = { engineeringLayers: layers };
...
export function wirePatternPrimaryColor(style: WireStyle): string {
  switch (style.pattern.kind) {
    case 'solid':
...
export function choosePaletteColor(index: number): string {
