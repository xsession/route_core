...
export const DEFAULT_COMPONENT_LAYOUT: ComponentLayoutRules = {
...
export const DEFAULT_COMPONENT_STYLE: ComponentVisualStyle = {
  bodyFill: '#ffffff',
  bodyStroke: '#4b5563',
  bodyStrokeWidth: 1.5,
  bodyRadius: 4,
...
export class ApproximateTextMeasurer implements TextMeasurer {
  public measure(text: string, style: Partial<TextStyle> = {}): TextMetrics {
...
function visibleBankPorts(
  ...
  const ports = bank.portIds
    ...
    .filter((value): value is PortSpec => Boolean(value?.visible))
    .filter((port) => !bank.collapseEmpty || connectedPortIds.has(port.id));
  if (bank.flow === 'reverse') return [...ports].reverse();
 ...
 */
function normalizedBanks(component: ComponentNode): PinBankSpec[] {
  const lookup = new Map(component.ports.map((port) => [port.id, port]));
  const assigned = new Set<EntityId>();
  const banks: PinBankSpec[] = [];

  for (const bank of component.pinBanks) {
    const portIds: EntityId[] = [];
    for (const portId of bank.portIds) {
      const port = lookup.get(portId);
      if (!port || assigned.has(portId) || port.side !== bank.side) continue;
      portIds.push(portId);
      assigned.add(portId);
    }
    if (portIds.length) banks.push({ ...bank, portIds });
  }
...
const PORT_LABEL_STYLE: Partial<TextStyle> = {
  ...DEFAULT_TEXT_STYLE,
  fontSize: 10.5,
  lineHeight: 13,
};

const PORT_FUNCTION_STYLE: Partial<TextStyle> = {
  ...DEFAULT_TEXT_STYLE,
  fontSize: 9.5,
  lineHeight: 12,
  fill: '#64748b',
...
interface PortLayoutItem {
  bank: PinBankSpec;
  port: PortSpec;
  labelMetrics: TextMetrics;
  functionMetrics?: TextMetrics;
  alongSize: number;
  textCrossSize: number;
  gapBefore: number;
}

interface SideLayoutProfile {
  side: Side;
  items: PortLayoutItem[];
  packedLength: number;
  crossSize: number;
  edgePadding: number;
...
interface ComponentLayoutProfile {
  sides: Record<Side, SideLayoutProfile>;
  titleMetrics: TextMetrics;
  subtitleMetrics: TextMetrics;
}

function measurePortText(
  port: PortSpec,
  measurer: TextMeasurer,
  labelStyle: Partial<TextStyle>,
  functionStyle: Partial<TextStyle>,
): { label: TextMetrics; fn?: TextMetrics; stackHeight: number; maximumWidth: number } {
...
function layoutProfile(
  component: ComponentNode,
  banks: PinBankSpec[],
  options: Required<Pick<ComponentGeometryOptions, 'connectedPortIds'>> & ComponentGeometryOptions,
  ...
  const sides: Record<Side, SideLayoutProfile> = {
    ...
    for (const bank of sideBanks) {
      ...
      for (let index = 0; index < ports.length; index += 1) {
        ...
        const alongSize = verticalSide
          ? Math.max(
              component.layout.rowHeight,
              port.minimumRowHeight ?? 0,
              measured.stackHeight + PORT_ROW_VERTICAL_PADDING * 2,
            )
...
function calculateAutoSize(component: ComponentNode, profile: ComponentLayoutProfile): Size {
  ...
  let requiredWidth = Math.max(
    component.layout.minimumSize.width,
    titleWidth,
    sideColumnsWidth,
    horizontalPinsWidth,
  );

  const verticalPinsHeight = Math.max(
    west.requiredLength,
    east.requiredLength,
    component.layout.rowHeight + Math.max(component.layout.padding.top, component.layout.padding.bottom) * 2,
  );
  let requiredHeight = Math.max(
    component.layout.minimumSize.height,
    north.crossSize
  ...
  if (component.size && component.layout.preserveManualSize) {
    if (!component.layout.autoWidth) requiredWidth = component.size.width;
    else requiredWidth = Math.max(requiredWidth, component.size.width);
    if (!component.layout.autoHeight) requiredHeight = component.size.height;
    else requiredHeight = Math.max(requiredHeight, component.size.height);
  }
 ...
 */
function linearCenters(items: PortLayoutItem[], start: number, end: number): number[] {
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    if (index > 0) cursor += item.gapBefore + automaticExtraGap;
    automatic.push(cursor + item.alongSize / 2);
    cursor += item.alongSize;
  }
  if (items.length === 1) automatic[0] = start + span / 2;

  if (!items.some((item) => item.port.sideFraction !== undefined)) return automatic;
...
function worldTextGeometry(
  localCenter: Point,
  metrics: TextMetrics,
  size: Size,
  component: ComponentNode,
): { point: Point; bounds: Rect } {
...
function verticalTextCenters(item: PortLayoutItem, centerY: number): { label: Point; fn?: Point } {
...
function horizontalTextCenters(
  item: PortLayoutItem,
  centerX: number,
  side: 'north' | 'south',
  size: Size,
): { label: Point; fn?: Point } {
  const stackHeight = item.labelMetrics.height + (item.functionMetrics ? PORT_TEXT_LINE_GAP + item.functionMetrics.height : 0);
...
export function buildComponentGeometry(
  component: ComponentNode,
  options: ComponentGeometryOptions = {},
): ComponentGeometry {
  const connectedPortIds = options.connectedPortIds ?? new Set<EntityId>();
  ...
  const headerHeight = Math.min(
    component.layout.headerHeight,
    Math.max(0, size.height - northBandHeight - southBandHeight - component.layout.footerHeight),
  );
  ...
  const worldPorts: Record<EntityId, PortGeometry> = {};
  const rowBounds: Record<EntityId, Rect> = {};
  const minimumHit = options.minimumPortHitSize ?? 18;

  const contentTop = headerTop + headerHeight + component.layout.padding.top;
  const contentBottom = Math.max(
    contentTop,
    footerTop - component.layout.padding.bottom,
  );
  ...
  for (const side of ['north', 'east', 'south', 'west'] as const) {
    ...
    for (let index = 0; index < sideProfile.items.length; index += 1) {
      ...
      let localRow: Rect;
      let textCenters: { label: Point; fn?: Point };
      if (side === 'west') {
        ...
        if (textCenters.fn && item.functionMetrics) textCenters.fn.x = PORT_TEXT_EDGE_GAP + item.functionMetrics.width / 2;
      } else if (side === 'east') {
        ...
        if (textCenters.fn && item.functionMetrics) textCenters.fn.x = size.width - PORT_TEXT_EDGE_GAP - item.functionMetrics.width / 2;
      } else if (side === 'north') {
  ...
  const subtitlePoint = component.labels.subtitle
    ? transformLocalPoint(
        { x: size.width / 2, y: headerTop + headerHeight / 2 + 8 },
        size,
        component.position,
        component.rotation,
        component.mirrorX,
...
export function componentPortMap(component: ComponentNode): Map<EntityId, PortSpec> {
  return new Map(component.ports.map((port) => [port.id, port]));
}

export function componentCenter(component: ComponentNode): Point {
  return { ...component.position };
}

export function cloneComponent(component: ComponentNode): ComponentNode {
  return JSON.parse(JSON.stringify(component)) as ComponentNode;
}

export function moveComponent(component: ComponentNode, delta: Point): ComponentNode {
  return { ...component, position: { x: component.position.x + delta.x, y: component.position.y + delta.y } };
}

export function rotateComponent(component: ComponentNode, clockwise = true): ComponentNode {
  const delta = clockwise ? 90 : 270;
  return { ...component, rotation: ((component.rotation + delta) % 360) as 0 | 90 | 180 | 270 };
}

export function resizeComponent(component: ComponentNode, size: Size): ComponentNode {
...
export class ComponentBuilder {
  private readonly node: ComponentNode;

  public constructor(id: EntityId, designator: string, kind: ComponentKind = 'connector') {
    this.node = {
      id,
      kind,
      designator,
      ...
      ports: [],
      pinBanks: [],
      layout: JSON.parse(JSON.stringify(DEFAULT_COMPONENT_LAYOUT)) as ComponentLayoutRules,
      locked: false,
      hidden: false,
      zIndex: 0,
    };
  ...
  public definition(id: string): this {
  ...
  public withLabels(labels: Partial<ComponentLabelFields>): this {
    this.node.labels = { ...this.node.labels, ...labels };
    return this;
  }

  public withLayout(layout: Partial<ComponentLayoutRules>): this {
    this.node.layout = {
      ...this.node.layout,
      ...layout,
  ...
  public withStyle(style: Partial<ComponentVisualStyle>): this {
  ...
  public rotate(rotation: 0 | 90 | 180 | 270): this {
    this.node.rotation = rotation;
    return this;
  }

  public addBank(bank: Omit<PinBankSpec, 'portIds'> & { portIds?: EntityId[] }): this {
    this.node.pinBanks.push({ ...bank, portIds: [...(bank.portIds ?? [])] });
    return this;
  }

  public addPort(input: AddPortInput): this {
...
export function bodyAnchorPoint(geometry: ComponentGeometry, side: Side, offset = 0): Point {
...
export function localBodyCenter(geometry: ComponentGeometry): Point {
