import type { Axis, Point, Rect, RouteSegment } from '../types.js';
...
export function removeDuplicatePoints(points: Point[]): Point[] {
  ...
  for (const current of points) {
    if (result.length === 0 || !pointsEqual(result[result.length - 1], current)) result.push({ ...current });
  }
  return result;
}
...
export function segmentsFromPoints(points: Point[]): RouteSegment[] {
  const segments: RouteSegment[] = [];
  for (let index = 1; index < points.length; index += 1) {
    const start = points[index - 1];
    const end = points[index];
    const axis: Axis | 'diagonal' = start.y === end.y ? 'horizontal' : start.x === end.x ? 'vertical' : 'diagonal';
    segments.push({ start, end, axis, length: distance(start, end) });
  }
  return segments;
}

export function pointAtFraction(points: Point[], fraction: number): { point: Point; tangent: Point; segmentIndex: number } {
  if (points.length === 0) return { point: { x: 0, y: 0 }, tangent: { x: 1, y: 0 }, segmentIndex: 0 };
  if (points.length === 1) return { point: { ...points[0] }, tangent: { x: 1, y: 0 }, segmentIndex: 0 };
  const total = polylineLength(points);
  if (total === 0) return { point: { ...points[0] }, tangent: { x: 1, y: 0 }, segmentIndex: 0 };
...
export function distanceToPolyline(value: Point, points: Point[]): number {
  if (points.length === 0) return Number.POSITIVE_INFINITY;
  if (points.length === 1) return distance(value, points[0]);
...
export function roundedPath(points: Point[], requestedRadius: number): RoundedPath {
  const clean = removeDuplicatePoints(points);
  if (clean.length === 0) return { d: '', corners: [] };
  if (clean.length === 1) return { d: `M ${formatPoint(clean[0])}`, corners: [] };
  ...
  const corners: RoundedPathCorner[] = [];
...
export function allSegmentsAxisAligned(points: Point[]): boolean {
  for (let index = 1; index < points.length; index += 1) {
    if (!isAxisAligned(points[index - 1], points[index])) return false;
  }
  return true;
}

export function segmentIntersectsRect(start: Point, end: Point, value: Rect): boolean {
  if (start.x === end.x) {
