...
export function rotatePointAround(value: Point, center: Point, rotation: OrthogonalRotation): Point {
...
export function inverseRotation(rotation: OrthogonalRotation): OrthogonalRotation {
  return rotation === 90 ? 270 : rotation === 270 ? 90 : rotation;
}

export function rotateRectAround(value: Rect, center: Point, rotation: OrthogonalRotation): Rect {
...
export function rotateSide(side: Side, rotation: OrthogonalRotation): Side {
  const sequence: Side[] = ['north', 'east', 'south', 'west'];
  const index = sequence.indexOf(side);
  return sequence[(index + rotation / 90) % 4];
}

export function localToWorld(local: Point, origin: Point, size: Size, rotation: OrthogonalRotation): Point {
...
export function worldToLocal(world: Point, origin: Point, size: Size, rotation: OrthogonalRotation): Point {
  const center = { x: origin.x + size.width / 2, y: origin.y + size.height / 2 };
