...
import { snapComponentDrag, type SnapGuide } from './snapping.js';

export type InteractionState =
  | { kind: 'idle' }
  | { kind: 'marquee'; pointerId: number; start: Point; current: Point; additive: boolean; subtractive: boolean }
  | { kind: 'drag-components'; pointerId: number; start: Point; componentIds: EntityId[] }
  | { kind: 'drag-label'; pointerId: number; start: Point; labelId: EntityId }
  | { kind: 'drag-wire-segment'; pointerId: number; start: Point; wireId: EntityId; segmentIndex: number; originalPoints: Point[] }
  | { kind: 'connect-wire'; pointerId: number; source: WireEndpoint; current: Point; target?: WireEndpoint; valid: boolean; reason?: string }
  | { kind: 'pan'; pointerId: number; start: Point; current: Point };

export interface InteractionEventMap {
  stateChanged: { state: InteractionState };
  marqueeChanged: { bounds?: Rect; mode?: 'window' | 'crossing' };
  connectionPreview: { source?: WireEndpoint; target?: WireEndpoint; point?: Point; valid?: boolean; reason?: string };
  panRequested: { delta: Point };
  cursorRequested: { cursor: string };
  contextMenuRequested: { point: Point; hit?: HitResult };
  snapGuidesChanged: { guides: SnapGuide[] };
}
...
function selectionFromHit(hit: HitResult): SelectionRef | undefined {
  switch (hit.kind) {
    case 'component-body':
...
function portEndpointFromHit(hit: HitResult): WireEndpoint | undefined {
  if (hit.kind !== 'port' || typeof hit.subId !== 'string') return undefined;
  return { kind: 'port', componentId: hit.entityId, portId: hit.subId };
}

function marqueeBounds(start: Point, current: Point): Rect {
  return rect(Math.min(start.x, current.x), Math.min(start.y, current.y), Math.abs(current.x - start.x), Math.abs(current.y - start.y));
}

function modifiersAdd(modifiers: ModifierState): boolean {
  return modifiers.shift || modifiers.ctrl || modifiers.meta;
}

export class EditorInteractionController {
  private stateValue: InteractionState = { kind: 'idle' };
  private readonly events = new TypedEventEmitter<InteractionEventMap>();
  private readonly dragThresholdPx: number;
  private componentDragStartGeometries?: Record<EntityId, ComponentGeometry>;

  public constructor(
    private readonly engine: HarnessEditorEngine,
    private readonly options: InteractionControllerOptions,
  ) {
    this.dragThresholdPx = options.dragThresholdPx ?? 3;
  }

  public get state(): Readonly<InteractionState> {
    return this.stateValue;
  }

  public on<TKey extends keyof InteractionEventMap>(type: TKey, listener: (event: InteractionEventMap[TKey]) => void): () => void {
    return this.events.on(type, listener);
  }

  public pointerDown(event: EditorPointerEvent): void {
    ...
    if (!primary) {
      ...
      if (!modifiersAdd(event.modifiers) && !event.modifiers.alt) this.engine.clearSelection();
    ...
    if (selection) {
      if (modifiersAdd(event.modifiers)) this.engine.toggleSelection(selection);
      else if (!this.engine.selection.items.some((item) => item.kind === selection.kind && item.id === selection.id && item.subId === selection.subId)) this.engine.select([selection], selection);
    }
    ...
    if ((primary.kind === 'component-body' || primary.kind === 'component-header') && !this.engine.document.components[primary.entityId]?.locked) {
      ...
      this.componentDragStartGeometries = deepClone(this.engine.geometries as Record<EntityId, ComponentGeometry>);
  ...
  public pointerMove(event: EditorPointerEvent): void {
    ...
    if (event.pointerId !== this.stateValue.pointerId) return;
    switch (this.stateValue.kind) {
      ...
      case 'drag-components': {
        ...
        const snapshot = this.componentDragStartGeometries ?? (this.engine.geometries as Record<EntityId, ComponentGeometry>);
        const snapResult = event.modifiers.alt
          ? { delta: proposedDelta, guides: [] as SnapGuide[] }
  ...
  public pointerUp(event: EditorPointerEvent): void {
    if (this.stateValue.kind === 'idle' || event.pointerId !== this.stateValue.pointerId) return;
    const state = this.stateValue;
    switch (state.kind) {
      case 'marquee': {
        const bounds = marqueeBounds(state.start, event.point);
        const mode = event.point.x >= state.start.x ? 'window' : 'crossing';
        const selected = marqueeSelect(this.engine.document as EditorDocument, bounds, mode, {
          componentGeometries: this.engine.geometries as never,
          labelPlacements: [...this.engine.labelPlacements],
        });
        if (state.additive) this.engine.select([...this.engine.selection.items, ...selected]);
        ...
        } else this.engine.select(selected);
        this.events.emit('marqueeChanged', {});
        break;
      }
      case 'drag-components':
      ...
      case 'connect-wire':
        ...
        } else if (state.reason) this.engine.announce(state.reason);
        this.events.emit('connectionPreview', {});
        break;
      case 'pan':
        break;
  ...
  public pointerCancel(pointerId: number): void {
    if (this.stateValue.kind === 'idle' || this.stateValue.pointerId !== pointerId) return;
    if (this.stateValue.kind === 'drag-components' || this.stateValue.kind === 'drag-label' || this.stateValue.kind === 'drag-wire-segment') this.engine.cancelPreview();
    this.componentDragStartGeometries = undefined;
    this.events.emit('snapGuidesChanged', { guides: [] });
    this.events.emit('marqueeChanged', {});
    this.events.emit('connectionPreview', {});
  ...
  public keyDown(key: string, modifiers: ModifierState): boolean {
    const normalized = key.toLowerCase();
    if (normalized === 'escape') {
      if (this.stateValue.kind !== 'idle') this.pointerCancel(this.stateValue.pointerId);
    ...
    if (normalized === 'r' && this.stateValue.kind === 'idle') {
      ...
      if (ids.length > 0) this.engine.rotateComponents(ids, !modifiers.shift);
      return ids.length > 0;
    }
    if ((normalized === 'delete' || normalized === 'backspace') && this.stateValue.kind === 'idle') {
      const items = [...this.engine.selection.items];
      for (const item of items) {
        if (item.kind === 'wire') this.engine.removeWire(item.id);
        if (item.kind === 'label') this.engine.removeLabel(item.id);
        if (item.kind === 'component') this.engine.removeComponent(item.id, modifiers.shift ? 'detach' : 'prevent');
      }
  ...
  private hitsAt(point: Point): HitResult[] {
    return hitTestDocument(this.engine.document as EditorDocument, point, {
